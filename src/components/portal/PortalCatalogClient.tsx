...
export function PortalCatalogClient({ products }: Props) {
    ...
    <div>
      ...
      <div className="mt-4 mb-3 flex items-center justify-between">
        <p className="text-xs text-ink-3">
          {filtered.length === 0
            ? 'No products found'
            : `${filtered.length} product${filtered.length !== 1 ? 's' : ''}${query ? ` for "${query}"` : ''}`}
        </p>
        {(query || category !== ALL) && (
          <button
            type="button"
      ...
      {products.length === 0 ? (
        <div className="flex flex-col items-center py-20 text-center">
          ...
          <h2 className="mt-4 text-base font-semibold text-ink">No products available</h2>
          <p className="mt-1 text-sm text-ink-3">The catalogue is being stocked. Check back soon.</p>
        </div>
      ) : filtered.length === 0 ? (
        <div className="flex flex-col items-center py-20 text-center">
          <div className="grid h-16 w-16 place-items-center rounded-full bg-bg-muted text-ink-3">
