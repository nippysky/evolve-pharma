...
// ─── CSS custom properties type for confetti ───────────────────────────────
...
// In production, pull from session; demo hardcode is fine for the demo meeting.
const DEMO_EMAIL           = 'adaeze.nwosu@greenleafpharmacy.ng';
...
function buildFd(form: FormFields, pay: PayValue, paystackRef?: string): FormData {
    ...
    <div className="relative overflow-hidden rounded-3xl border border-teal-100 bg-gradient-to-b from-teal-50/80 via-white to-white px-6 py-16 text-center shadow-[0_20px_80px_rgba(45,212,191,0.12)]">
      ...
      >
        <div
          ...
          style={{ animation: 'ring-ping 1s ease-out 0.65s both' }}
        />
        <div
          className="absolute inset-0 rounded-full bg-teal-300"
          style={{ animation: 'ring-ping 1s ease-out 0.82s both' }}
        />
      ...
      {/* Timeline */}
      <div
        className="mx-auto mt-10 max-w-xs text-left"
        style={{ animation: 'fade-up 0.5s ease-out 0.75s both' }}
      >
        <p className="mb-5 text-center text-[10px] font-semibold uppercase tracking-[0.16em] text-ink-3">
          What happens next
        </p>
          ...
          {steps.map(({ emoji, label, desc }, i) => (
            <div key={label} className="flex items-start gap-4">
              ...
              <div className="flex flex-col items-center">
                <span className="grid h-9 w-9 shrink-0 place-items-center rounded-full border-2 border-teal-200 bg-white text-base shadow-sm">
                  {emoji}
                </span>
                {i < steps.length - 1 && (
                  <div className="my-1 w-px flex-1 bg-teal-100" style={{ minHeight: 24 }} />
                )}
              </div>
...
function StepHeader({
  n, label, optional, children,
}: {
  n: number; label: string; optional?: boolean; children: React.ReactNode;
}) {
  return (
    <div className="flex items-center gap-2.5">
      <span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-[#042a36] text-[11px] font-bold text-white shadow-[0_4px_12px_rgba(4,42,54,0.22)]">
        {n}
      </span>
      <span className="text-teal-600">{children}</span>
      <h2 className="text-base font-semibold tracking-tight text-ink">{label}</h2>
      {optional && (
        <span className="rounded-full bg-bg-muted px-2 py-0.5 text-[10px] font-medium text-ink-3">
          Optional
        </span>
      )}
    </div>
...
// ─── FieldRow wrapper — shows validation indicator ─────────────────────────
function ValidatedField({
  label, htmlFor, required, error, hint, touched, valid, children,
}: {
...
function EmptyBasket() {
  return (
    <div className="rounded-3xl border border-line bg-white py-24 text-center">
      <span className="mx-auto mb-5 grid h-20 w-20 place-items-center rounded-full bg-bg-muted text-ink-3">
        <Box size={32} />
      </span>
...
export default function CheckoutPage() {
  ...
  const change = useCallback((field: keyof FormFields, value: string) => {
    setForm((f) => {
      ...
      if (touched[field]) {
        setErrors((e) => ({ ...e, [field]: validateOne(field, value, next) ?? undefined }));
      }
      return next;
    });
  }, [touched]);
  ...
  const blur = useCallback((field: keyof FormFields) => {
    ...
    setErrors((e) => ({
      ...e,
      [field]: validateOne(field, form[field], form) ?? undefined,
    }));
  }, [form]);
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!runValidation()) {
      // Scroll to the first visible error so user notices it
      setTimeout(() => {
    ...
    if (pay === 'paystack') {
      ...
      try {
        const handler = window.PaystackPop.setup({
          ...
          // Paystack rejects async functions — use a sync wrapper with a void IIFE inside
          callback: (response) => {
            void (async () => {
              setSubmitting(true);
              try {
                const result = await checkoutAction(null, buildFd(form, pay, response.reference));
                if (result.ok) {
                  const num = (result.data as { order_number: string }).order_number;
                  handleSuccess(num);
                } else {
                  setPageError(
                ...
                setPageError(
                  `Network error. Payment succeeded (ref: ${response.reference}) — please contact support.`,
                );
              } finally {
                setSubmitting(false);
              }
        ...
        setPageError(`Could not open payment: ${msg}. Check your Paystack key and try again.`);
        console.error('[Paystack] setup error:', err);
      }

    } else {
      // ── Bank transfer / Pay-on-delivery flow ──
      setSubmitting(true);
      try {
        const result = await checkoutAction(null, buildFd(form, pay));
        if (result.ok) {
          const num = (result.data as { order_number: string }).order_number;
          handleSuccess(num);
        } else {
  ...
  }, [pay, psLoaded, form, total, runValidation, handleSuccess]);
  ...
  const isFieldValid = (field: keyof FormFields) =>
  ...
  return (
    <>
      ...
      <PageHead
        title="Checkout"
        subtitle="Complete your delivery details and payment to place your order."
      />
      ...
      <form onSubmit={handleSubmit} noValidate className="grid gap-6 lg:grid-cols-[1fr_390px]">
        ...
        <div className="flex flex-col gap-4">
          ...
          {cancelled && (
            <div className="flex items-start gap-3 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3.5 text-sm text-amber-800 shadow-sm">
              ...
              <button
                ...
                className="shrink-0 rounded-lg p-0.5 text-amber-400 transition-colors hover:bg-amber-100 hover:text-amber-600"
              >
                <X size={14} />
              </button>
            </div>
          ...
          <section className="rounded-2xl border border-line bg-white p-6 shadow-[0_4px_24px_rgba(15,23,42,0.04)]">
            <StepHeader n={1} label="Delivery address">
              <MapPin size={15} />
            </StepHeader>
            ...
            <div className="mt-5 grid gap-4">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <ValidatedField
                  label="State" htmlFor="state" required
                  error={errors.state} touched={touched.state} valid={isFieldValid('state')}
                >
                  <Select
                    id="state" name="state"
                    value={form.state}
                    ...
                    className={cn(
                      ...
                      touched.state && errors.state
                        ? 'border-red-400 bg-red-50/50 focus:border-red-400 focus:shadow-[0_0_0_3px_rgba(248,113,113,0.14)]'
                        : touched.state && !errors.state
                ...
                <ValidatedField
                  label="City / LGA" htmlFor="city" required
                  error={errors.city} touched={touched.city} valid={isFieldValid('city')}
                >
                  <Input
                    id="city" name="city"
                    value={form.city}
              ...
              {/* Street address */}
              <ValidatedField
                label="Street address" htmlFor="street_address" required
                error={errors.street_address} touched={touched.street_address}
              >
                <Textarea
                  id="street_address" name="street_address" rows={2}
                  value={form.street_address}
                  onChange={(e) => change('street_address', e.target.value)}
                  onBlur={() => blur('street_address')}
              ...
              {/* Phone */}
              <ValidatedField
                label="Contact phone" htmlFor="contact_phone" required
                error={errors.contact_phone} touched={touched.contact_phone}
                valid={isFieldValid('contact_phone')}
              >
                <div className="relative">
                  <span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 select-none text-xs font-semibold text-ink-3">
                    +234
                  </span>
                  <Input
                    ...
                    className={cn(
                      'pl-12 transition-all duration-150',
                  ...
                  {!touched.contact_phone && (
                    <Phone size={13} className="pointer-events-none absolute right-3.5 top-1/2 -translate-y-1/2 text-ink-4" />
                  )}
                </div>
              </ValidatedField>
              ...
              {/* Delivery notes */}
              <Field
                label="Delivery notes" htmlFor="delivery_notes"
                hint="Gate code, preferred delivery window, building instructions, etc."
              >
                <Textarea
                  id="delivery_notes" name="delivery_notes" rows={2}
                  value={form.delivery_notes}
                  onChange={(e) => change('delivery_notes', e.target.value)}
          ...
          <section className="rounded-2xl border border-line bg-white p-6 shadow-[0_4px_24px_rgba(15,23,42,0.04)]">
            <StepHeader n={2} label="Payment method">
              <CreditCard size={15} />
            </StepHeader>
              ...
              {PAY_METHODS.map(({ value, label, sub, Icon, recommended }) => {
                ...
                return (
                  <label
                    key={value}
                    className={cn(
                      'relative flex cursor-pointer flex-col gap-3 rounded-2xl border p-4 transition-all duration-200',
                      active
                        ? 'border-teal-400 bg-gradient-to-br from-teal-50 to-cyan-50/60 shadow-[0_0_0_3px_rgba(45,212,191,0.14),0_8px_28px_rgba(45,212,191,0.14)]'
                        : 'border-line bg-white hover:border-teal-200 hover:shadow-[0_4px_18px_rgba(15,23,42,0.07)]',
                    )}
                  >
                    <input
                      type="radio" name="payment_method" value={value}
                      checked={active} onChange={() => setPay(value)}
                      className="sr-only"
                    />
                    <div className="flex items-start justify-between">
                      ...
                      {active && (
                        <span className="grid h-5 w-5 place-items-center rounded-full bg-teal-500 text-white shadow-[0_2px_8px_rgba(20,184,166,0.55)]">
                          <Check size={10} />
                        </span>
                      )}
                    </div>
                    ...
                    {recommended && (
                      <span className="absolute right-2.5 top-2.5 rounded-full bg-teal-500 px-2 py-[3px] text-[9px] font-bold uppercase tracking-wide text-white shadow-[0_2px_8px_rgba(20,184,166,0.4)]">
                        Recommended
                      </span>
                    )}
                  </label>
            ...
            <div className="mt-4">
              {pay === 'paystack' && (
                <div className="flex items-center gap-3 rounded-xl border border-teal-100 bg-teal-50 px-4 py-2.5">
                  ...
                  <p className="text-xs text-teal-700">
                    <strong className="font-semibold">Secured by Paystack</strong>
                    {' '}· 256-bit SSL · PCI DSS Level 1 compliant
                  </p>
                </div>
              )}
              {pay === 'bank_transfer' && (
                <div className="rounded-xl border border-line bg-bg-subtle px-4 py-3 text-xs leading-relaxed text-ink-2">
              ...
              {pay === 'cash_on_delivery' && (
                <div className="rounded-xl border border-amber-100 bg-amber-50 px-4 py-3 text-xs leading-relaxed text-amber-800">
                  Pay on delivery is available for verified pharmacy accounts in select areas only.
                  Cash or POS accepted on arrival.
                </div>
              )}
            </div>
          </section>

          {/* ── Step 3: Purchase reference ── */}
          <section className="rounded-2xl border border-line bg-white p-6 shadow-[0_4px_24px_rgba(15,23,42,0.04)]">
            <StepHeader n={3} label="Purchase reference" optional>
              <Box size={15} />
            </StepHeader>
            <div className="mt-5">
              <Field
                label="Purchase order #" htmlFor="po_number"
                hint="Appears on your invoice and delivery note."
              >
                <Input
                  id="po_number" name="po_number"
                  value={form.po_number}
                  onChange={(e) => change('po_number', e.target.value)}
        ...
        <aside className="sticky top-20 self-start rounded-2xl border border-line bg-white p-6 shadow-[0_10px_50px_rgba(15,23,42,0.08)]">
            ...
            <ul className="flex flex-col gap-3 border-b border-line pb-4">
              {items.map((item) => (
                <li key={item.product_id} className="flex items-start gap-3">
                  <span className="grid h-8 w-8 shrink-0 place-items-center rounded-xl bg-bg-muted text-ink-3">
                    <Box size={14} />
                  </span>
          ...
          <div className="mt-4 space-y-2.5">
            ...
            <SummaryRow
              ...
              valueClass={shipping === 0 ? 'text-teal-600 font-semibold' : ''}
            />
            <SummaryRow label="VAT (7.5%)" value={formatNaira(vat)} />
          </div>

          {/* Total strip */}
          <div className="mt-4 flex items-baseline justify-between rounded-2xl bg-[#042a36] px-5 py-3.5">
            <span className="text-sm font-semibold text-white/70">Total</span>
            <span className="num font-display text-2xl font-bold tracking-tight text-white">
              {formatNaira(total)}
            </span>
          </div>
          ...
          {/* Free shipping nudge */}
          {shipping > 0 && (
            <p className="mt-3 rounded-xl border border-amber-100 bg-amber-50 px-3.5 py-2.5 text-xs leading-relaxed text-amber-700">
              Add{' '}
              <strong className="font-semibold">{formatNaira(FREE_SHIP_THRESHOLD - subtotal)}</strong>{' '}
          ...
          <button
            ...
            className={cn(
              ...
              'hover:-translate-y-0.5 hover:bg-teal-900 hover:shadow-[0_16px_40px_rgba(4,42,54,0.36)]',
          ...
          >
            {submitting ? (
              ...
              <>
                {pay === 'paystack' ? <CreditCard size={15} /> : <Lock size={15} />}
                {pay === 'paystack'
